...
$download_heading = $download_heading ?? t('panel_download_heading_default');
$download_intro = $download_intro ?? t('panel_download_intro_default');
...
<head>
    ...
    <style>
        ...
        * { scrollbar-width: thin; scrollbar-color: var(--border-glow) var(--bg); }
        ...
        .verify-spin { animation: verifySpin 0.9s linear infinite; flex-shrink: 0; }
        @keyframes verifySpin { from { transform: rotate(0deg); } to { transform: rotate(360deg); } }
    ...
    <main class="container">
        ...
        <div class="card">
            ...
            <div class="instruction-list">
                ...
                <?= empty_state_box(t('panel_empty_instructions')) ?>
                <?php endif; ?>
        ...
        <div style="display:flex; flex-direction:column; gap:24px;">
            ...
            <div class="card">
                ...
                <div class="person-list" id="person-list">
                    ...
                    <?= empty_state_box(t('panel_empty_persons')) ?>
                    <?php endif; ?>
                    <!-- Generated by JS -->
                </div>
            </div>
            ...
            <div class="card" style="flex:1; display:flex; flex-direction:column;">
                ...
                <div class="key-counter">
                    ...
                    <div class="result-verifying" id="result-verifying">
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="verify-spin"><path d="M21 12a9 9 0 1 1-6.219-8.56"/></svg>
                        <?= htmlspecialchars(t('panel_secret_verifying')) ?>
                    </div>
        ...
        <div class="card" id="dl-card" style="grid-column: 1 / -1;">
            <div id="dl-inner-content"<?= in_array($tlStatus['state'], ['pending', 'blocked'], true) ? ' class="dl-blurred"' : '' ?>>
                ...
                <?= empty_state_box(t('panel_empty_downloads')) ?>
                <?php else: ?>
...
(function (root, factory) {
    ...
    var I18N_JS = {
        ...
        secretRejected:     '<?= addslashes(t('panel_secret_rejected')) ?>',
        secretRateLimited:  '<?= addslashes(t('panel_secret_rate_limited')) ?>',
        secretNetworkError: '<?= addslashes(t('panel_secret_network_error')) ?>',
        decryptFailed:      '<?= addslashes(t('panel_decrypt_failed')) ?>'
    };
    ...
    var resultVerifying = document.getElementById('result-verifying');

    function showLocked() {
        resultLocked.style.display = 'flex';
        resultValue.style.display = 'none';
        resultError.style.display = 'none';
        resultVerifying.style.display = 'none';
    }
    ...
    function showVerifying() {
        ...
        resultValue.style.display = 'none';
        resultError.style.display = 'none';
        resultVerifying.style.display = 'flex';
    }
    ...
    partsInput.addEventListener('input', function() {
        ...
        try {
            ...
            if (combined !== null && (verifiedSubsets || verifiedFormat)) {
                var combinedTrimmed = combined.trim();
                // Nie pokazujemy jeszcze hasła — dopóki serwer nie potwierdzi zgodności
                // z SECRET_HASH, nie wiemy, czy to, co się złożyło, jest prawdziwe. Stan
                // pośredni ("Weryfikuję...") jest neutralny (niebieski), żeby nikt nie
                // wziął go przez pomyłkę za potwierdzenie poprawności.
                showVerifying();
                cancelFailLog();
                clearTimeout(window._decryptLogTimer);
                window._decryptLogTimer = setTimeout(function() {
                    // Znacznik "generacji" próby — jeśli w międzyczasie (zanim wróci odpowiedź
                    // serwera) użytkownik zdąży wpisać/wkleić coś innego i wywoła kolejną,
                    // nowszą próbę, ta stara odpowiedź nie może już nadpisać świeżo pokazanej
                    // wartości ani komunikatu — ignorujemy ją jako nieaktualną.
                    window._armAttemptSeq = (window._armAttemptSeq || 0) + 1;
                    var myAttemptId = window._armAttemptSeq;
                    ...
                    fetch('arm-timelock.php', { method: 'POST', headers: {'Content-Type': 'application/json'},
                        body: JSON.stringify({ secret: combinedTrimmed, csrf_token: CSRF_TOKEN }) })
                        .then(function(r) { return r.json(); })
                        .then(function(res) {
                            if (!res || myAttemptId !== window._armAttemptSeq) return;
                            // Serwer zweryfikował hasło jako NIEPRAWIDŁOWE — nigdy nie pokazujemy
                            // go w ogóle, tylko czytelny komunikat.
                            if (res.rejected) { showError(I18N_JS.secretRejected); return; }
                            if (res.rate_limited) { showError(I18N_JS.secretRateLimited); return; }
                            // W każdym innym przypadku hasło jest zaufane — albo świeżo zweryfikowane
                            // (res.armed), albo timelock był już wcześniej uzbrojony/zablokowany
                            // (serwer w ogóle nie sprawdzał hasła, bo to i tak niepotrzebne — patrz
                            // idempotentny guard w arm-timelock.php). Dopiero teraz je pokazujemy.
                            showValue(combinedTrimmed);
                            if (res.armed && res.unlock_at) activateDlPendingState(res.unlock_at);
                        })
                        .catch(function() {
                            // Brak połączenia z serwerem — nie możemy potwierdzić, więc NIE pokazujemy
                            // hasła (lepiej czytelny błąd niż niezweryfikowana wartość udawana za pewną).
                            if (myAttemptId === window._armAttemptSeq) showError(I18N_JS.secretNetworkError);
                        });
                }, 1500);
            } else {
                showError(I18N_JS.decryptFailed);
