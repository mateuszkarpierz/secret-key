...
**アクセス層**（`/decrypt/`）
- `index.php` — JS内でのShamir再構築を伴う復号パネル。例外が発生しなかったというだけで信用するのではなく、2つの独立した方法（断片の部分集合間の数学的整合性＋テキストフォーマット分析）で結果を検証
- `download.php` — 制御されたファイルダウンロード（セッションが必要、設定から構築されたホワイトリスト、サーバー側のログ記録）＋パネルから独立した厳格なタイムロック検証
- `log.php` — イベントのログ記録
- `devtools-log.php` — DevTools検査の試みをログ記録（IPごとのレート制限あり）
- `timelock.php` — 受託者共謀後のタイムロックロジック（[セキュリティ](#セキュリティ)を参照）
- `arm-timelock.php` — パスワード復元確認後に48時間ロックを起動し、アラートメールを送信
- `panic.php` — メール内のワンタイム「Panic Button」リンクを処理
- `tl-status.php` — ロック状態のライブポーリング（パネルの再読み込み不要）
...
- `timelock.json` — 受託者共謀後の48時間ロック状態*（自動作成、[セキュリティ](#セキュリティ)を参照）*
...
このシステムは**9つの独立した保護層**を組み合わせており、1つが破られても、システムへのアクセスは得られません。
...
> [!TIP]
| 🚨 **受託者共謀への対策** | 48時間タイムロック + Panic Button + サーバー側検証 | パスワードの最初の復元成功（オプションで `SECRET_HASH` によりサーバー側で検証）でファイルダウンロードを48時間ブロックし、即座かつ永久にアクセスをブロックするワンタイムリンク付きのアラートメールを送信します——詳細は下記 |

> **存命中の受託者による共謀への対策。** Shamirのアルゴリズムは、数学的には、指定された受託者が結託すればマスターパスワード自体を復元できてしまいます——これはこのシステムに限らず、あらゆる閾値秘密分散方式に避けられない性質です。システムはこれに対して3つのレベルで対応します：（1）ブラウザ側で結合された断片の**整合性をチェック**します（断片の部分集合間の数学的整合性＋結果フォーマットの分析）——これによりタイプミスやランダムなノイズは除外されますが、それだけでは結合されたシークレットが本物のパスワードであることを**確認したことにはなりません**（ログイン済みの人物が、完全に整合性のある、しかし捏造された断片を、まったく別の「パスワード」として貼り付けることが可能です）。（2）設定でオプションですが強く推奨される `SECRET_HASH`（ダッシュボードが自動計算する本物のパスワードのbcryptハッシュ）を設定すると、サーバーは何かを起動する前に、`password_verify()` を通じて結合されたシークレットを**実際に検証**します。（3）この検証の後にのみ、**ファイルダウンロードを48時間ブロック**し、ワンタイムの「Panic Button」リンク付きのメールを送信します——誰かが何かをダウンロードする前に、一度のクリックで永久にアクセスを遮断できます。パスワードを知っているだけでは、データベースの実ファイルなしには何の価値もありません。最大限の保護のためには、パスワードデータベース自体の第二要素としてハードウェアキー（例：YubiKey/FIDO2）を使用することも追加で推奨されます。
...
`dashboard.html` をブラウザでローカルに開きます。3つのタブがあります。

**設定** — `secret-key.php` ファイルを生成します：
1. SMSPlanetのAPIトークン、SMS送信者名、コードの自動入力用ドメイン（Android/iOS）— ドメインのみで、`@`や`https://`は不要
2. 復元パネルで必要なコードのしきい値（K）— 暗号化タブで設定したしきい値と一致させる必要があります — および、任意ですが推奨：本物のマスターパスワード（暗号化タブで使用したものと同じ）。ダッシュボードがこれからサーバー側検証用のハッシュを自動計算します（[受託者共謀への対策](#セキュリティ)を参照）
3. 指定する各人について：ログイン、パスワード、名、姓、電話番号、パネルの所持者リストに表示するかどうか
4. ダウンロード可能なファイル（パスワードデータベース、2FAデータベース、プログラムのインストーラー）— キー、ボタンのラベル、ファイル名
5. パネル用の手順（フォーマット：`**太字**` / `*イタリック*`）
6. オプション：ログインごとのメール通知（受信者アドレス、送信者、パネルへのリンク）
7. 「設定を生成」をクリックし、生成された `secret-key.php` ファイルをダウンロード
...
**カードジェネレーター** — 印刷可能な、パーソナライズされたSecret Keyカードを作成します（ISO ID-1規格、85.6×54mm）：
1. グローバル設定：インスタンスのURLと所有者の氏名
2. 分割パラメーター（断片の総数と必要な最小数）— 「設定」および「暗号化」タブと一致させる必要があります
3. カード裏面の編集可能なメッセージ — 断片数、しきい値、インスタンスURLを自動挿入するトークン付き
4. 各人について：ログイン、パスワード、Shamirの断片（16進数）— 前の2つのタブのデータ
5. オプション：印刷所での印刷用に2mmの塗り足しとトンボ（裁ち落とし線）
6. 「カードを生成」をクリック — 各カードの表と裏のプレビューが表示され、PDFとしてダウンロード可能

> [!TIP]
> 3つのツールすべてが**完全にオフライン**で動作し、データはブラウザから決して離れません。設定フォームは実行ごとに `secret-key.php` をゼロから生成します — 既存のファイルの読み込みや編集は行いません。
...
<details>
<summary><strong>存命中に受託者が結託し、私の知らないうちにパスワードを復元してしまったらどうなりますか？</strong></summary>

パスワードだけでは彼らには不十分です。データベースファイルはサーバーの公開ディレクトリの外に置かれており、そこへのアクセスは`download.php`が制御しています。パネルでパスワードが初めて正しく復元された瞬間——オプションですが強く推奨されるのは、`SECRET_HASH` によりサーバー側でも検証されることです。これにより、結合された断片の整合性だけ（本物のパスワードを知らなくても）ではロックを強制するのに不十分になります——システムは自動的にファイルのダウンロードを48時間ブロックし、ワンタイムの「Panic Button」リンク付きのアラートメールをあなたに送信します——一度クリックするだけで永久にアクセスを遮断し、落ち着いてマスターパスワードを変更する時間を確保できます。さらにハードウェアキー（例：YubiKey）でパスワードデータベースを保護している場合、ファイルのロックが解除された後でも、パスワードを知っているだけではそれを開くのに十分ではありません。
...
このプロジェクトが役に立ったなら、⭐ を残してください——必要としている他の人がこのプロジェクトを見つける助けになります。
