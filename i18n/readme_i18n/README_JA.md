...
> [!TIP]
> **存命中の受託者による共謀への対策。** Shamirのアルゴリズムは、数学的には、指定された受託者が結託すればマスターパスワード自体を復元できてしまいます——これはこのシステムに限らず、あらゆる閾値秘密分散方式に避けられない性質です。パスワードデータベースをハードウェアキー（例：YubiKey/FIDO2）で追加保護していない場合、復元されたパスワードだけでデータベースを完全に開けてしまいます。Secret Keyはログインおよびファイルダウンロード試行時のメール通知プローブによってこのリスクを軽減していますが、最大限の保護のためには、パスワードデータベース自体の第二要素としてハードウェアキーを使用することを推奨します——そうすれば、マスターパスワードを知っているだけでは、鍵が物理的にあなたのそばになければ誰の役にも立ちません。
