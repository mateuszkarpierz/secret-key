...
> [!TIP]
| 🚨 **受託者共謀への対策** | 48時間タイムロック + Panic Button | パスワードの最初の復元成功でファイルダウンロードを48時間ブロックし、即座かつ永久にアクセスをブロックするワンタイムリンク付きのアラートメールを送信します——詳細は下記 |

> **存命中の受託者による共謀への対策。** Shamirのアルゴリズムは、数学的には、指定された受託者が結託すればマスターパスワード自体を復元できてしまいます——これはこのシステムに限らず、あらゆる閾値秘密分散方式に避けられない性質です。システムはこれに対して2つのレベルで対応します：（1）復元されたパスワードが、誤った断片から生じた暗号学的なゴミデータではなく本物であることを**検証**します（断片の部分集合間の数学的整合性＋結果フォーマットの分析）。（2）復元成功が確認された後、**ファイルダウンロードを48時間ブロック**し、ワンタイムの「Panic Button」リンク付きのメールを送信します——誰かが何かをダウンロードする前に、一度のクリックで永久にアクセスを遮断できます。パスワードを知っているだけでは、データベースの実ファイルなしには何の価値もありません。最大限の保護のためには、パスワードデータベース自体の第二要素としてハードウェアキー（例：YubiKey/FIDO2）を使用することも追加で推奨されます。
