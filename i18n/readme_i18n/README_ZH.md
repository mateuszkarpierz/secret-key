...
> [!TIP]
| 🚨 **防止委托人串通** | 48小时 Timelock + Panic Button | 首次成功重建密码会阻止文件下载 48 小时，并发送一封带有一次性链接的警报邮件，可立即永久阻止访问——详见下文 |

> **防止委托人在你生前串通。** Shamir 算法在数学上允许被指定的委托人在串通的情况下重建主密码——这是任何门限秘密共享方案不可避免的特性，并非本系统独有。系统在两个层面应对这一点：（1）**验证**恢复出的密码是否真实，而不是来自错误分片的密码学垃圾数据（跨分片子集的数学一致性 + 结果格式分析）；（2）在确认重建成功后，**阻止文件下载 48 小时**，并向你发送一封带有一次性「Panic Button」链接的邮件——点击一次即可在任何人下载任何文件之前永久切断访问。仅仅知道密码，没有数据库实体文件毫无用处。为获得最高级别的保护，还建议将硬件密钥（例如 YubiKey/FIDO2）作为密码库本身的第二重要素。
...
仅凭密码对他们来说是不够的。数据库文件存放在服务器公共目录之外，其访问由 `download.php` 控制。面板中第一次成功重建密码的那一刻，系统会自动阻止文件下载 48 小时，并向你发送一封警报邮件，其中包含一次性的「Panic Button」链接——点击一次即可永久切断访问，让你有时间从容更改主密码。如果你还用硬件密钥（例如 YubiKey）额外保护密码库，那么即使文件已解锁，仅凭密码也不足以打开它。
