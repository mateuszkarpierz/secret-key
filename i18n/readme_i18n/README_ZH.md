...
**访问层**（`/decrypt/`）
- `index.php` —— 带有 JS 端 Shamir 重新拼合功能的解密面板；通过两种独立方法验证结果（跨分片子集的数学一致性 + 文本格式分析），而不是仅凭没有抛出异常就予以信任
- `download.php` —— 受控文件下载（需要有效会话、基于配置生成的白名单、服务器端日志记录）+ 独立于面板的严格 timelock 校验
- `log.php` —— 事件日志记录
- `devtools-log.php` —— 记录 DevTools 检查行为的尝试（带按 IP 的速率限制）
- `timelock.php` —— 委托人串通后的 timelock 逻辑（见[安全性](#安全性)）
- `arm-timelock.php` —— 在确认密码重建成功后启动 48 小时锁定，发送警报邮件
- `panic.php` —— 处理邮件中的一次性「Panic Button」链接
- `tl-status.php` —— 实时轮询锁定状态（无需刷新面板）
...
- `timelock.json` —— 委托人串通后 48 小时锁定的状态*（自动创建，见[安全性](#安全性)）*
- `secret-key.log` —— 事件日志
...
该系统结合了**九层独立的防护机制**——攻破其中一层并不能获得系统的访问权限。
...
> [!TIP]
| 🚨 **防止委托人串通** | 48小时 Timelock + Panic Button + 服务器端验证 | 首次成功重建密码（可选经由 `SECRET_HASH` 在服务器端验证）会阻止文件下载 48 小时，并发送一封带有一次性链接的警报邮件，可立即永久阻止访问——详见下文 |

> **防止委托人在你生前串通。** Shamir 算法在数学上允许被指定的委托人在串通的情况下重建主密码——这是任何门限秘密共享方案不可避免的特性，并非本系统独有。系统在三个层面应对这一点：（1）浏览器端**检查一致性**——检查合并后的分片是否一致（跨分片子集的数学一致性 + 结果格式分析），这能过滤掉手误和随机噪声，但本身**并不能确认**合并出的密文就是真实密码（已登录的人完全可以粘贴一组内部完全一致、但凭空编造的分片，对应一个截然不同的"密码"）；（2）如果你在配置中设置了可选但强烈建议启用的 `SECRET_HASH`（由仪表盘自动计算的真实密码 bcrypt 哈希值），服务器会在激活任何机制之前，通过 `password_verify()` 对合并出的密文进行**真正的验证**；（3）只有通过该验证后，系统才会**阻止文件下载 48 小时**，并向你发送一封带有一次性「Panic Button」链接的邮件——点击一次即可在任何人下载任何文件之前永久切断访问。仅仅知道密码，没有数据库实体文件毫无用处。为获得最高级别的保护，还建议将硬件密钥（例如 YubiKey/FIDO2）作为密码库本身的第二重要素。
...
在浏览器中本地打开 `dashboard.html`。它包含三个标签页：

**配置** —— 生成 `secret-key.php` 文件：
1. SMSPlanet API 令牌、短信发送者名称，以及用于自动填充验证码（Android/iOS）的域名——只填域名本身，不含 `@` 和 `https://`
2. 恢复面板所需的验证码门限（K）——须与"加密"标签页中设置的门限一致——以及可选（但建议填写）的真实主密码（与"加密"标签页中使用的相同），仪表盘会据此自动计算用于服务器端验证的哈希值（参见[防止委托人串通](#安全性)）
3. 为每位指定人员填写：登录名、密码、名字、姓氏、手机号，以及是否在面板的持有人列表中显示
4. 可下载文件（密码数据库、双因素认证数据库、程序安装包）——键名、按钮文字、文件名
5. 面板中的操作说明步骤（支持格式：`**粗体**` / `*斜体*`）
6. 可选：每次登录时发送邮件通知（收件地址、发件人、面板链接）
7. 点击"生成配置"，下载生成的 `secret-key.php` 文件
...
**卡片生成器** —— 生成可直接打印的个性化 Secret Key 卡片（ISO ID-1 标准卡片尺寸，85.6×54mm）：
1. 全局设置：系统实例网址和所有者姓名
2. 拆分参数（密钥份额总数和所需最小数量）——需与"配置"和"加密"标签页保持一致
3. 卡片背面的可编辑留言，支持插入占位符自动填入份额数量、门限值和实例网址
4. 为每位人员填写：登录名、密码和 Shamir 密钥份额（十六进制）——数据来自前两个标签页
5. 可选：2mm 出血位和裁切线，方便送印刷厂打印
6. 点击"生成卡片"——预览每张卡片的正反面，可下载为 PDF

> [!TIP]
> 这三个工具都**完全离线运行**——没有任何数据离开浏览器。配置表单每次运行时都会从零生成新的 `secret-key.php`——它不会加载或编辑已有文件。
...
<details>
<summary><strong>如果委托人串通，在我生前未经我知晓就恢复了密码，会怎样？</strong></summary>

仅凭密码对他们来说是不够的。数据库文件存放在服务器公共目录之外，其访问由 `download.php` 控制。面板中第一次成功重建密码的那一刻——建议同时选配通过 `SECRET_HASH` 进行服务器端验证，这样仅凭合并分片的一致性（而不知道真实密码）就不足以强行触发锁定——系统会自动阻止文件下载 48 小时，并向你发送一封警报邮件，其中包含一次性的「Panic Button」链接——点击一次即可永久切断访问，让你有时间从容更改主密码。如果你还用硬件密钥（例如 YubiKey）额外保护密码库，那么即使文件已解锁，仅凭密码也不足以打开它。
...
如果这个项目对你有帮助，请留下一个 ⭐ —— 这能帮助其他可能需要它的人找到它。
