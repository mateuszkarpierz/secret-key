...
> [!TIP]
> **防止委托人在你生前串通。** Shamir 算法在数学上允许被指定的委托人在串通的情况下重建主密码——这是任何门限秘密共享方案不可避免的特性，并非本系统独有。如果你没有额外使用硬件密钥（例如 YubiKey/FIDO2）保护你的密码库，重建出的密码就足以让他们完全打开密码库。Secret Key 通过登录和文件下载尝试时的邮件通知探测来降低这一风险，但为了获得最高级别的保护，建议将硬件密钥作为密码库本身的第二重要素——这样，仅仅知道主密码，在没有密钥实物在你身边的情况下，对任何人都毫无用处。
