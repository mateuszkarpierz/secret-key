...
> [!TIP]
| 🚨 **Захист від змови довірених осіб** | Timelock 48г + Panic Button | Перша успішна реконструкція пароля блокує завантаження файлів на 48г і надсилає тривожний лист з одноразовим посиланням для негайного і постійного блокування доступу — див. нижче |

> **Захист від змови довірених осіб за вашого життя.** Алгоритм Шаміра математично дозволяє призначеним довіреним особам відновити сам головний пароль, якщо вони змовляться це зробити — це неминуча властивість будь-якої порогової схеми розподілу секрету, а не лише цієї. Система реагує на це на двох рівнях: (1) вона **перевіряє**, що відновлений пароль справжній, а не криптографічне сміття з неправильних часток (математична узгодженість між підмножинами часток + аналіз формату результату), і (2) після підтвердженої успішної реконструкції **блокує завантаження файлів на 48 годин**, надсилаючи вам лист з одноразовим посиланням «Panic Button» — один клік назавжди відрізає доступ, перш ніж хтось встигне щось завантажити. Саме лише знання пароля марне без фізичного файлу бази даних. Для максимального захисту додатково рекомендується використовувати апаратний ключ (наприклад, YubiKey/FIDO2) як другий фактор для самої бази паролів.
