...
Secret Key використовує бібліотеку [`secrets.js`](https://github.com/grempe/secrets.js) (форк [amper5and/secrets.js](https://github.com/amper5and/secrets.js), вбудований через [iancoleman.io/shamir](https://iancoleman.io/shamir/)) для розділення та відновлення секрету.
...
| Параметр | Значення |
|---|---|
| Бібліотека | secrets.js (форк grempe, через iancoleman.io/shamir) |
...
- `index.php` — панель розшифрування з відновленням за Шаміром на JS; перевіряє результат двома незалежними методами (математична узгодженість між підмножинами часток + аналіз формату тексту) замість того, щоб довіряти простій відсутності винятку
- `download.php` — контрольована видача файлів (потребує сесію, білий список, побудований із конфігурації, логування на боці сервера) + сувора перевірка timelock, незалежна від панелі
- `log.php` — логування подій
- `devtools-log.php` — логування спроб відкриття DevTools (з обмеженням частоти за IP)
- `timelock.php` — логіка timelock після змови довірених осіб (див. [Безпека](#безпека))
- `arm-timelock.php` — активує 48-годинне блокування після підтвердженої реконструкції пароля, надсилає тривожний лист
- `panic.php` — обробляє одноразове посилання «Panic Button» з листа
- `tl-status.php` — опитування стану блокування в реальному часі (без потреби перезавантажувати панель)
...
- `timelock.json` — стан 48-годинного блокування після змови довірених осіб *(створюється автоматично, див. [Безпека](#безпека))*
...
Система поєднує **дев'ять незалежних рівнів захисту** — компрометація одного з них не дає доступу до системи.

| Рівень | Механізм | Деталі |
|---|---|---|
| 🔒 **Паролі** | bcrypt | cost=10, формат `$2y$`, перевірка, стійка до атак за часом |
| 🛡️ **CSRF** | Токен із 64 hex-символів | Генерується криптографічно, перевіряється на кожній кінцевій точці, що змінює стан (вхід, перевірка 2FA, повторне надсилання SMS, вихід із системи, журнал подій) |
...
> [!TIP]
| 🚨 **Захист від змови довірених осіб** | Timelock 48г + Panic Button + перевірка на сервері | Перша успішна реконструкція пароля (опційно перевірена на сервері через `SECRET_HASH`) блокує завантаження файлів на 48г і надсилає тривожний лист з одноразовим посиланням для негайного і постійного блокування доступу — див. нижче |

> **Захист від змови довірених осіб за вашого життя.** Алгоритм Шаміра математично дозволяє призначеним довіреним особам відновити сам головний пароль, якщо вони змовляться це зробити — це неминуча властивість будь-якої порогової схеми розподілу секрету, а не лише цієї. Система реагує на це на трьох рівнях: (1) у браузері **перевіряється узгодженість** об'єднаних часток (математична узгодженість між підмножинами + аналіз формату результату) — це відсіює описки та випадковий шум, але саме собою **не підтверджує**, що об'єднаний секрет є справжнім паролем (авторизована особа може вставити повністю узгоджені, але вигадані частки з зовсім іншим «паролем»); (2) якщо ви задасте опційний, але настійно рекомендований `SECRET_HASH` у конфігурації (bcrypt-хеш справжнього пароля, автоматично обчислюваний дашбордом), сервер **справді перевіряє** об'єднаний секрет через `password_verify()`, перш ніж щось активувати; (3) лише після цієї перевірки система **блокує завантаження файлів на 48 годин**, надсилаючи вам лист з одноразовим посиланням «Panic Button» — один клік назавжди відрізає доступ, перш ніж хтось встигне щось завантажити. Саме лише знання пароля марне без фізичного файлу бази даних. Для максимального захисту додатково рекомендується використовувати апаратний ключ (наприклад, YubiKey/FIDO2) як другий фактор для самої бази паролів.
...
Відкрийте `dashboard.html` локально у браузері. Він містить три вкладки:
...
2. Поріг (K) потрібних кодів у панелі відновлення — має збігатися з порогом, заданим на вкладці Шифрування — а також, опційно, але рекомендовано: справжній головний пароль (той самий, що й на вкладці Шифрування), з якого дашборд сам обчислює хеш для перевірки на стороні сервера (див. [Захист від змови довірених осіб](#безпека))
3. Для кожної призначеної особи: логін, пароль, ім'я, прізвище, номер телефону та чи має вона бути видимою у списку власників на панелі
4. Файли для завантаження (база паролів, база 2FA, інсталятор програми) — ключ, підпис кнопки, ім'я файлу
5. Кроки інструкції для панелі (форматування: `**напівжирний**` / `*курсив*`)
6. Опційно: email-сповіщення при кожному вході (адреса отримувача, відправник, посилання на панель)
7. Натисніть «Згенерувати конфігурацію» та завантажте отриманий файл `secret-key.php`
...
**Генератор карток** — створює персоналізовані картки Secret Key, готові до друку (формат ISO ID-1, 85,6×54мм):
1. Глобальні налаштування: URL вашого екземпляра системи та повне ім'я власника
2. Параметри розподілу (загальна кількість фрагментів і потрібний мінімум) — мають збігатися з вкладками Конфігурація та Шифрування
3. Редаговане повідомлення на звороті картки з токенами для автоматичного вставлення кількості фрагментів, порогу та URL екземпляра
4. Для кожної особи: логін, пароль і фрагмент Шаміра (hex) — дані з двох попередніх вкладок
5. Опційно: припуск на обріз 2мм і лінії різу для друку в типографії
6. Натисніть «Згенерувати картки» — попередній перегляд лицьового та зворотного боку кожної картки, готової до завантаження у PDF

> [!TIP]
> Усі три інструменти працюють **повністю офлайн** — жодні дані не залишають браузер. Форма конфігурації генерує `secret-key.php` з нуля під час кожного запуску — вона не завантажує і не редагує наявний файл.
...
<details>
<summary><strong>Що якщо довірені особи змовляться і відновлять пароль за мого життя, без мого відома?</strong></summary>

Самого пароля їм не вистачить. Файли бази даних знаходяться за межами публічної директорії сервера, а доступ до них контролює `download.php`. У момент першої успішної реконструкції пароля в панелі — опційно, але настійно рекомендовано, також перевіреної на сервері через `SECRET_HASH`, щоб самої лише узгодженості об'єднаних часток (без знання справжнього пароля) було недостатньо для примусового блокування — система автоматично блокує завантаження файлів на 48 годин і надсилає вам тривожний лист з одноразовим посиланням «Panic Button» — один клік назавжди відрізає доступ, даючи вам час спокійно змінити головний пароль. Якщо ви додатково захищаєте базу паролів апаратним ключем (наприклад, YubiKey), самого знання пароля не вистачить, щоб її відкрити, навіть після розблокування файлів.
...
Якщо цей проєкт виявився корисним, залиште ⭐ — це допоможе іншим, кому він може знадобитися, знайти його.
